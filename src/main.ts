...
async function run(): Promise<void> {
  ...
  const initialDelaySeconds: number = parseInt(core.getInput("initial-delay-seconds", {required: true}));
  const intervalSeconds: number = parseInt(core.getInput("interval-seconds", {required: true}));
  const timeoutSeconds: number = parseInt(core.getInput("timeout-seconds", {required: true}));
  ...
  await loop(octokit, statusRegex, checkRunRegex, intervalSeconds, timeoutSeconds)
}
...
type FailedCheckRun    = CompletedCheckRun & {conclusion: "cancelled"|"failure"|"timed_out"}
...
async function loop(octokit: Octokit, statusRegex: RegExp, checkRunRegex: RegExp, intervalSeconds: number, timeoutSeconds: number) {
  ...
  do {
    ...
    const failedStatuses = completedStatuses.filter(isStatusFailed).map((status) => status.context);
    const failedCheckRuns = completedCheckRuns.filter(isCheckRunFailed).map((run) => run.name);
...
async function combinedStatusLoopIteration(octokit: Octokit, regex: RegExp): Promise<[PendingStatus[], CompletedStatus[]]> {
...
async function checkRunLoopIteration(octokit: Octokit, regex: RegExp): Promise<[PendingCheckRun[], CompletedCheckRun[]]> {
